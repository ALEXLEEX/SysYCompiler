...
!!! warning "使用最新的模板代码"
    学期初版本的模板代码存在一些 bug，因此强烈建议同学们检查自己基于的模板代码是最新的。尤其是提前开始试验的同学，因为模板代码在实验开始前可能仍有更新。
...
??? note "模板代码中的中间代码生成"
    ...
    ??? info "使用 OCaml 模板注意命名问题"
        OCaml 模板不在语义分析阶段对符号进行重命名，因此在中间代码生成阶段中你需要重新构建符号表，从当前符号名映射到新的唯一标识符，如有需要，你还需要映射到该符号的作用域等信息，在 OCaml 中你可以轻松的定义这样的类型并且通过泛型复用语义分析阶段中的符号表实现。
